import { ArrowRight, BadgeCheck, Globe2, TrendingDown, TrendingUp, Trophy } from "lucide-react";
...
export function RankingsPreview() {
  const featured = divisionRankings.slice(0, 4);
  ...
  return (
    <div>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link
          className="inline-flex min-h-12 items-center justify-center rounded-full border border-red-500/40 px-5 py-3 text-xs font-black uppercase tracking-[0.18em] text-red-100 transition hover:bg-red-500/10 sm:text-sm"
          href="/rankings"
        >
          View All Rankings
      ...
      <div className="mt-8 grid gap-5 sm:mt-10 lg:grid-cols-2 2xl:grid-cols-4">
        {featured.map((division) => (
          <DivisionRankingCard compact division={division} key={division.slug} />
        ))}
      </div>
    </div>
  );
...
function DivisionRankingCard({
  ...
  return (
    <article className="glass-panel overflow-hidden rounded-[1.5rem] sm:rounded-[1.75rem]">
