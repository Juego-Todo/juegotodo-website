...
import { Download, MoreHorizontal, Plus, Search, SlidersHorizontal, Upload } from "lucide-react";
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          <button
            className="inline-flex items-center gap-2 rounded-full bg-[#FF1010] px-4 py-3 text-[0.62rem] font-black uppercase tracking-[0.16em] text-white transition hover:bg-[#ff2828]"
            onClick={onCreate}
            type="button"
          >
          ...
          <div className="relative" ref={menuRef}>
            ...
            {menuOpen ? (
              <div className="absolute left-0 top-[calc(100%+0.5rem)] z-20 min-w-[12rem] overflow-hidden rounded-2xl border border-white/10 bg-[#111] shadow-2xl sm:left-auto sm:right-0">
                ...
                <button
                  className="flex w-full items-center gap-2 px-4 py-3 text-left text-sm text-zinc-300 hover:bg-white/5 hover:text-white"
                  onClick={() => {
                    onImport();
                    setMenuOpen(false);
                  }}
