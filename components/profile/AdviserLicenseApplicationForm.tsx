...
const stepTitles = ["Personal Information", "Professional Information", "Uploads & Preview"] as const;

type FormStep = 1 | 2 | 3;

type AdviserLicenseApplicationFormProps = {
  user: UserProfile;
  userPhone: string;
...
export function AdviserLicenseApplicationForm({
  user,
  userPhone,
  initialApplication,
  showConfirmationInitially,
}: AdviserLicenseApplicationFormProps) {
  ...
  const [form, setForm] = useState<LicenseApplicationInput>(() => {
    ...
    return {
      ...emptyLicenseApplicationInput,
      applicationProgram: "adviser_license",
      restrictionCode: "JT6",
  ...
  const previewIdentity = useMemo(
    () =>
      ({
        memberId: application?.idNumber ?? "ADV-PREVIEW",
  ...
  function validateStep2() {
    if (!form.backgroundAnswers.adviserPosition?.trim()) return "Position is required.";
    if (!form.backgroundAnswers.areaOfExpertise?.trim()) return "Area of expertise is required.";
    if (!form.backgroundAnswers.organization?.trim()) return "Organization is required.";
    if (!form.backgroundAnswers.yearsExperience?.trim()) return "Years experience is required.";
    return null;
  }

  function validateStep3() {
    if (!form.uploads.resume) return "Resume upload is required.";
  ...
  function handleEditApplication() {
    ...
    setStepError(null);
    setError(null);
    router.replace("/register-for-license/adviser");
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Unable to submit adviser license application.");
    } finally {
      setSubmitting(false);
    }
  }
          ...
          <MotionSection>
            ...
            <div className="glass-panel mt-6 rounded-[1.5rem] p-8 text-center sm:p-10">
              ...
              <p className="mt-4 text-sm leading-7 text-zinc-400">
                {buildLicenseFullName(form)} • {form.backgroundAnswers.adviserPosition}
              </p>
              <p className="mt-2 text-xs font-black uppercase tracking-[0.16em] text-violet-300">
                Adviser License ID will be issued as {application.idNumber} upon approval
              </p>
              ...
              {!isApproved ? (
                <button
                  className="mt-6 inline-flex min-h-11 items-center justify-center rounded-full border border-white/15 px-8 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:border-violet-400/40 hover:bg-white/5"
    ...
    <main className="overflow-hidden px-4 pt-24 sm:px-6 sm:pt-28 lg:px-8 lg:pt-32">
      <section className="relative mx-auto max-w-3xl py-8 sm:py-10">
        <MotionSection>
          ...
          <div className="mt-5">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-violet-400">JTGC Adviser License</p>
            <h1 className="font-display mt-2 text-3xl uppercase text-white sm:text-4xl">Adviser Application</h1>
          </div>
          ...
          <form className="glass-panel mt-6 rounded-[1.5rem] p-5 sm:p-6" onSubmit={handleSubmit}>
            <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4">
              <div>
                <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-violet-400">
              ...
              <div className="hidden gap-1 sm:flex">
                {Array.from({ length: TOTAL_STEPS }, (_, index) => (
                  <span
                    className={`h-1.5 w-6 rounded-full ${step >= index + 1 ? "bg-violet-500" : "bg-white/10"}`}
                    key={index}
                  />
                ))}
              </div>
            ...
            <div className="py-4">
              {step === 1 ? (
                <FormGroup
                  columns={3}
                  description="Enter your personal details for your official JTGC adviser license."
              ...
              {step === 2 ? (
                <FormGroup
                  description="Provide your professional role, expertise, and organizational affiliation."
                  title="Professional Information"
                >
                  <Field
                    className="sm:col-span-2"
                    label="Position"
                    onChange={(value) => updateBackgroundField("adviserPosition", value)}
                    placeholder="e.g. Senior Adviser, Legal Adviser, Technical Adviser"
                    required
                    value={form.backgroundAnswers.adviserPosition ?? ""}
                  />
                  <Field
                    className="sm:col-span-2"
                    label="Area of Expertise"
                    onChange={(value) => updateBackgroundField("areaOfExpertise", value)}
                    placeholder="e.g. Sports Law, Martial Arts Development, Governance"
                    required
                    value={form.backgroundAnswers.areaOfExpertise ?? ""}
                  />
                  <Field
                    className="sm:col-span-2"
                    label="Organization"
                    onChange={(value) => updateBackgroundField("organization", value)}
                    placeholder="Affiliated organization or council body"
                    required
                    value={form.backgroundAnswers.organization ?? ""}
                  />
                  <Field
                    label="Years Experience"
                    onChange={(value) => updateBackgroundField("yearsExperience", value)}
                    placeholder="e.g. 12"
                    required
                    type="number"
                    value={form.backgroundAnswers.yearsExperience ?? ""}
                  />
                </FormGroup>
              ) : null}
              ...
              {step === 3 ? (
                <div className="space-y-4">
                  <FormGroup description="Upload your resume, government ID, and signature." title="Uploads">
                    <FileUploadField
                      accept="image/*,.pdf"
                      className="sm:col-span-2"
                      label="Resume"
                      onFileSelect={(file) => updateUploadField("resume", file)}
                      required
                      value={form.uploads.resume}
                    />
                    <FileUploadField
                      accept="image/*,.pdf"
                      label="Government ID"
                  ...
                  <section className="rounded-2xl border border-violet-500/15 bg-black/20 p-4 sm:p-5">
                    <div className="mb-4 border-b border-white/8 pb-3">
                      <h3 className="text-xs font-black uppercase tracking-[0.18em] text-violet-300">Adviser License Preview</h3>
                      <p className="mt-1.5 text-xs leading-5 text-zinc-500">
                        Review your official JTGC adviser credential before submission. Drag to rotate the card.
                      </p>
                    </div>
                    <AdviserLicenseIdPreview form={form} identity={previewIdentity} user={user} />
                  </section>
                </div>
              ) : null}
            </div>
            ...
            <div className="space-y-3 border-t border-white/10 pt-4">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {step < TOTAL_STEPS ? (
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-violet-600 px-8 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:bg-violet-500"
                    onClick={handleNext}
                    type="button"
                  >
                    Next
                  ...
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-violet-600 px-8 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:bg-violet-500 disabled:opacity-60"
                    disabled={submitting}
                    type="submit"
                  >
