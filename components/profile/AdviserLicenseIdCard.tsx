...
function CardShell({ children, rotateY = 0 }: { children: ReactNode; rotateY?: number }) {
  return (
    <div
      className="absolute inset-0 overflow-hidden rounded-[0.85rem] border border-violet-500/25 shadow-[0_24px_70px_rgba(0,0,0,0.45)] [backface-visibility:hidden]"
...
function FieldBlock({
  ...
  return (
    <div>
      <p className={`text-[0.48rem] font-black uppercase tracking-[0.08em] text-black ${accent ? "text-violet-700" : ""}`}>
        {label}
      </p>
      <p
...
function AdviserCardFront({
  ...
  const position = showDetails && application ? answers.adviserPosition || application.positionTitle || "—" : "—";
  const expertise = showDetails && application ? answers.areaOfExpertise || "—" : "—";
  const organization = showDetails && application ? answers.organization || application.fightTeam || "—" : "—";
  const yearsExperience = showDetails && application ? answers.yearsExperience || "—" : "—";
    ...
    <div className="flex h-full bg-white text-black">
      <div className="relative flex w-[34%] flex-col bg-[#efefef] p-2">
        <div className="flex flex-1 items-center justify-center border border-black/15 bg-white">
          {showDetails ? (
            <div className="flex h-full w-full items-center justify-center bg-[linear-gradient(180deg,#8b5cf6,#6d28d9)] font-display text-4xl text-white">
              {initials}
            </div>
          ) : (
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-2">
          ...
          <div className="grid h-10 w-10 place-items-center rounded-sm bg-violet-700 text-[0.42rem] font-black leading-none text-white">
            AD
          </div>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between bg-violet-950 px-3 py-2 text-white">
          <div>
            <p className="font-display text-[1.05rem] leading-none tracking-[0.04em]">JUEGO TODO</p>
            <p className="text-[0.55rem] font-semibold tracking-[0.18em] text-violet-200">ADVISER LICENSE</p>
          </div>
          <div
            aria-hidden
        ...
        <div className="relative flex-1 overflow-hidden bg-[#ececec] p-3">
          <div className="relative space-y-2">
            ...
            <FieldBlock label="Area of Expertise" value={expertise} />
            <FieldBlock label="Organization" value={organization} />
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Adviser ID" value={idNumber} />
              <FieldBlock label="Years Experience" value={yearsExperience} />
            </div>
            <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-violet-800 px-4 py-1.5">
          <p className="font-display text-[1.15rem] italic leading-none tracking-[0.08em] text-white">Adviser</p>
        </div>
      </div>
    </div>
  );
...
function AdviserCardBack({
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-[#f2f2f2] text-black">
      <div className="bg-violet-950 px-3 py-2">
        <p className="text-[0.55rem] font-black uppercase tracking-[0.12em] text-violet-100">Professional Credentials</p>
      </div>

      <div className="flex-1 px-3 py-3">
        <div className="space-y-1 text-[0.38rem]">
          <BackField label="Position" value={showDetails ? answers.adviserPosition || "" : ""} />
          <BackField label="Expertise" value={showDetails ? answers.areaOfExpertise || "" : ""} />
          <BackField label="Organization" value={showDetails ? answers.organization || "" : ""} />
          <BackField label="Years Experience" value={showDetails ? answers.yearsExperience || "" : ""} />
      ...
      <div className="border-t border-black/10 bg-violet-950 px-3 py-2 text-center">
        <p className="text-[0.34rem] leading-[0.52rem] text-violet-100">
          Authorized JTGC adviser credential. Valid for sanctioned advisory and council support activities.
        </p>
      </div>
    </div>
  );
...
export function AdviserLicenseIdCard({
  user,
  identity,
  application = null,
  preview = false,
  ...
  return (
    <div className="w-full max-w-[28rem]">
      ...
      >
        ...
        >
          <CardShell>
            <AdviserCardFront application={application} approved={approved} preview={preview} user={user} />
          </CardShell>
          <CardShell rotateY={180}>
            <AdviserCardBack application={application} approved={approved} preview={preview} />
          </CardShell>
        </div>
      </div>

      {preview ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-violet-300 lg:text-left">
          Preview — Adviser License pending approval
        </p>
      ) : !showApprovedState ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-zinc-500 lg:text-left">
          {application?.status === "pending"
            ? "Awaiting adviser license approval"
            : application?.status === "rejected"
              ? "Adviser license not approved — resubmit required"
              : "Submit your Adviser License application to unlock credentials"}
        </p>
      ) : (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-violet-300 lg:text-left">
          Official JTGC Adviser License • {identity.memberId}
        </p>
      )}
    </div>
  );
