...
const stepTitles = ["Personal Information", "Club Information", "Business Information", "Uploads & Preview"] as const;

type FormStep = 1 | 2 | 3 | 4;

type ClubOwnerApplicationFormProps = {
  user: UserProfile;
  userPhone: string;
...
export function ClubOwnerApplicationForm({
  user,
  userPhone,
  initialApplication,
  showConfirmationInitially,
}: ClubOwnerApplicationFormProps) {
  ...
  const previewIdentity = useMemo(
    () =>
      ({
        memberId: application?.idNumber ?? "CLB-PREVIEW",
  ...
  function validateStep2() {
    if (!form.backgroundAnswers.clubName?.trim()) return "Club name is required.";
    if (!form.backgroundAnswers.clubAddress?.trim()) return "Club address is required.";
    if (!form.backgroundAnswers.clubProvince?.trim()) return "Province is required.";
    if (!form.backgroundAnswers.clubRegion?.trim()) return "Region is required.";
    if (!form.backgroundAnswers.establishedYear?.trim()) return "Established year is required.";
    if (!form.backgroundAnswers.numberOfMembers?.trim()) return "Number of members is required.";
    return null;
  }

  function validateStep3() {
    if (!form.backgroundAnswers.dtiRegistration?.trim()) return "DTI registration is required.";
    return null;
  }

  function validateStep4() {
    if (!form.uploads.clubLogo) return "Club logo upload is required.";
  ...
  function handleEditApplication() {
    ...
    setStepError(null);
    setError(null);
    router.replace("/register-for-license/club-owner");
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Unable to submit club owner application.");
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <section className="relative mx-auto max-w-2xl py-8 sm:py-10">
          <MotionSection>
            ...
            <div className="glass-panel mt-6 rounded-[1.5rem] p-8 text-center sm:p-10">
              <div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-zinc-500/15 text-zinc-200">
                <CheckCircle2 size={32} aria-hidden />
              </div>
              <h1 className="font-display mt-6 text-4xl uppercase text-white">Application Submitted</h1>
              <p className="mt-4 text-sm leading-7 text-zinc-400">
                {form.backgroundAnswers.clubName} • {buildLicenseFullName(form)}
              </p>
              <p className="mt-2 text-xs font-black uppercase tracking-[0.16em] text-zinc-300">
                Club Owner ID will be issued as {application.idNumber} upon approval
              </p>
    ...
    <main className="overflow-hidden px-4 pt-24 sm:px-6 sm:pt-28 lg:px-8 lg:pt-32">
      <section className="relative mx-auto max-w-3xl py-8 sm:py-10">
        <MotionSection>
          ...
          <div className="mt-5">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-zinc-400">Affiliated Club</p>
            <h1 className="font-display mt-2 text-3xl uppercase text-white sm:text-4xl">Club Owner Application</h1>
          </div>
          ...
          <form className="glass-panel mt-6 rounded-[1.5rem] p-5 sm:p-6" onSubmit={handleSubmit}>
            <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4">
              <div>
                <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-zinc-400">
              ...
              <div className="hidden gap-1 sm:flex">
                {Array.from({ length: TOTAL_STEPS }, (_, index) => (
                  <span
                    className={`h-1.5 w-6 rounded-full ${step >= index + 1 ? "bg-zinc-300" : "bg-white/10"}`}
                    key={index}
                  />
                ))}
              </div>
            ...
            <div className="py-4">
              {step === 1 ? (
                <FormGroup
                  columns={3}
                  description="Enter your personal details as the registered club owner."
              ...
              {step === 2 ? (
                <FormGroup description="Provide details about your affiliated club or gym." title="Club Information">
                  <Field
                    ...
                    label="Club Name"
                    onChange={(value) => updateBackgroundField("clubName", value)}
                    required
                    value={form.backgroundAnswers.clubName ?? ""}
                  />
                  <Field
                    className="sm:col-span-2"
                    label="Club Address"
                    onChange={(value) => updateBackgroundField("clubAddress", value)}
                    required
                    value={form.backgroundAnswers.clubAddress ?? ""}
                  />
                  <Field
                    label="Province"
                    onChange={(value) => updateBackgroundField("clubProvince", value)}
                    required
                    value={form.backgroundAnswers.clubProvince ?? ""}
                  />
                  <SelectField
                    label="Region"
                    onChange={(value) => updateBackgroundField("clubRegion", value)}
                    options={philippineRegions}
                    required
                    value={form.backgroundAnswers.clubRegion ?? ""}
                  />
                  <Field
                    label="Established Year"
                    onChange={(value) => updateBackgroundField("establishedYear", value)}
                    placeholder="e.g. 2018"
                    required
                    type="number"
                    value={form.backgroundAnswers.establishedYear ?? ""}
                  />
                  <Field
                    label="Number of Members"
                    onChange={(value) => updateBackgroundField("numberOfMembers", value)}
                    placeholder="e.g. 45"
                    required
                    type="number"
                    value={form.backgroundAnswers.numberOfMembers ?? ""}
                  />
                  <Field
                    label="Website"
                    onChange={(value) => updateBackgroundField("website", value)}
                    placeholder="https://"
                    type="url"
                    value={form.backgroundAnswers.website ?? ""}
                  />
                  <Field
                    label="Facebook Page"
                    onChange={(value) => updateBackgroundField("facebookPage", value)}
                    placeholder="facebook.com/yourclub"
                    value={form.backgroundAnswers.facebookPage ?? ""}
                  />
                </FormGroup>
              ) : null}
              ...
              {step === 3 ? (
                <FormGroup description="Enter your club's business registration details." title="Business Information">
                  <Field
                    ...
                    onChange={(value) => updateBackgroundField("dtiRegistration", value)}
                    required
                    value={form.backgroundAnswers.dtiRegistration ?? ""}
                  />
                  <Field
                    className="sm:col-span-2"
                    label="SEC Registration"
                    onChange={(value) => updateBackgroundField("secRegistration", value)}
                    placeholder="Optional"
                    value={form.backgroundAnswers.secRegistration ?? ""}
                  />
                </FormGroup>
              ) : null}
              ...
              {step === 4 ? (
                <div className="space-y-4">
                  <FormGroup description="Upload your club logo, valid government ID, and signature." title="Uploads">
                    <FileUploadField
                      accept="image/*"
                      className="sm:col-span-2"
                      label="Club Logo"
                      onFileSelect={(file) => updateUploadField("clubLogo", file)}
                      required
                      value={form.uploads.clubLogo}
                    />
                    <FileUploadField
                      accept="image/*,.pdf"
                      label="Government ID"
                  ...
                  <section className="rounded-2xl border border-white/10 bg-black/20 p-4 sm:p-5">
                    <div className="mb-4 border-b border-white/8 pb-3">
                      <h3 className="text-xs font-black uppercase tracking-[0.18em] text-zinc-200">Club Owner ID Preview</h3>
                      <p className="mt-1.5 text-xs leading-5 text-zinc-500">
                        Review your affiliated club credential before submission. Drag to rotate the card.
                      </p>
                    </div>
                    <ClubOwnerIdPreview form={form} identity={previewIdentity} user={user} />
                  </section>
                </div>
              ) : null}
            </div>
            ...
            <div className="space-y-3 border-t border-white/10 pt-4">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {step < TOTAL_STEPS ? (
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-zinc-100 px-8 text-xs font-black uppercase tracking-[0.16em] text-black transition hover:bg-white"
                    onClick={handleNext}
                    type="button"
                  >
                    Next
                  ...
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-zinc-100 px-8 text-xs font-black uppercase tracking-[0.16em] text-black transition hover:bg-white disabled:opacity-60"
                    disabled={submitting}
                    type="submit"
                  >
