...
import { buildLicenseAddress, formatFighterRecord, formatLicenseDate } from "@/data/license-applications";
...
function CardShell({ children, rotateY = 0 }: { children: ReactNode; rotateY?: number }) {
  return (
    <div
      className="absolute inset-0 overflow-hidden rounded-[0.85rem] border border-red-500/25 shadow-[0_24px_70px_rgba(0,0,0,0.45)] [backface-visibility:hidden]"
...
function FieldBlock({
  ...
  return (
    <div>
      <p className={`text-[0.48rem] font-black uppercase tracking-[0.08em] text-black ${accent ? "text-red-700" : ""}`}>
        {label}
      </p>
      <p
...
function FighterCardFront({
  ...
  const nickname = showDetails && application ? answers.nickname || "—" : "—";
  const weightDivision = showDetails && application ? answers.weightDivision || application.positionTitle || "—" : "—";
  const record = showDetails && application ? formatFighterRecord(answers) || "—" : "—";
  const fightTeam = showDetails && application ? answers.fightTeam || application.fightTeam || "—" : "—";
  const gym = showDetails && application ? answers.gym || "—" : "—";
  const coach = showDetails && application ? answers.coachName || "—" : "—";
  const stance = showDetails && application ? answers.stance || "—" : "—";
  const dominantHand = showDetails && application ? answers.dominantHand || "—" : "—";
  ...
  return (
    <div className="flex h-full bg-white text-black">
      <div className="relative flex w-[34%] flex-col bg-[#efefef] p-2">
        <div className="flex flex-1 items-center justify-center border border-black/15 bg-white">
          {showDetails && profilePhoto ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-[linear-gradient(180deg,#FF1010,#991b1b)] font-display text-4xl text-white">
              {initials}
            </div>
          ) : (
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-2">
          ...
          <div className="grid h-10 w-10 place-items-center rounded-sm bg-[#FF1010] text-[0.42rem] font-black leading-none text-white">
            FT
          </div>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between bg-red-950 px-3 py-2 text-white">
          <div>
            <p className="font-display text-[1.05rem] leading-none tracking-[0.04em]">JUEGO TODO</p>
            <p className="text-[0.55rem] font-semibold tracking-[0.18em] text-red-200">FIGHTER LICENSE</p>
          </div>
          <div
            aria-hidden
        ...
        <div className="relative flex-1 overflow-hidden bg-[#ececec] p-3">
          <div className="relative space-y-2">
            ...
            <FieldBlock accent label="Fight Name" value={nickname} />
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock accent label="Weight Division" value={weightDivision} />
              <FieldBlock label="Record" value={record} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Fight Team" value={fightTeam} />
              <FieldBlock label="Gym" value={gym} />
            </div>
            <FieldBlock label="Coach" value={coach} />
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Stance" value={stance} />
              <FieldBlock label="Dominant Hand" value={dominantHand} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Fighter ID" value={idNumber} />
              <FieldBlock label="Years Training" value={showDetails && application ? answers.yearsTraining || "—" : "—"} />
            </div>
            <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-[#FF1010] px-4 py-1.5">
          <p className="font-display text-[1.15rem] italic leading-none tracking-[0.08em] text-white">Fighter</p>
        </div>
      </div>
    </div>
  );
...
function FighterCardBack({
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-[#f2f2f2] text-black">
      <div className="bg-red-950 px-3 py-2">
        <p className="text-[0.55rem] font-black uppercase tracking-[0.12em] text-red-100">Fighter Credentials</p>
      </div>

      <div className="flex-1 px-3 py-3">
        <div className="space-y-1 text-[0.38rem]">
          <BackField label="Height" value={showDetails ? answers.fighterHeight || "" : ""} />
          <BackField label="Weight" value={showDetails ? answers.fighterWeight || "" : ""} />
          <BackField label="Reach" value={showDetails ? answers.reach || "" : ""} />
          <BackField label="Blood Type" value={showDetails && application ? application.bloodType || "" : ""} />
          <BackField label="Medical Clearance" value={showDetails && answers.medicalClearance === "yes" ? "Confirmed" : ""} />
          <BackField label="ECG" value={showDetails && answers.medicalEcg === "yes" ? "Confirmed" : ""} />
          <BackField label="Blood Test" value={showDetails && answers.medicalBloodTest === "yes" ? "Confirmed" : ""} />
          ...
          <BackField label="Address" value={showDetails && application ? buildLicenseAddress(application) : ""} />
        </div>
      </div>

      <div className="border-t border-black/10 bg-red-950 px-3 py-2 text-center">
        <p className="text-[0.34rem] leading-[0.52rem] text-red-100">
          Authorized JTGC fighter credential. Valid only for sanctioned Juego Todo competition and registration activities.
        </p>
      </div>
    </div>
  );
...
export function FighterLicenseIdCard({
  user,
  identity,
  application = null,
  preview = false,
  ...
  return (
    <div className="w-full max-w-[28rem]">
      ...
      >
        ...
        >
          <CardShell>
            <FighterCardFront application={application} approved={approved} preview={preview} user={user} />
          </CardShell>
          <CardShell rotateY={180}>
            <FighterCardBack application={application} approved={approved} preview={preview} />
          </CardShell>
        </div>
      </div>

      {preview ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-red-300 lg:text-left">
          Preview — Fighter License pending approval
        </p>
      ) : !showApprovedState ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-zinc-500 lg:text-left">
          {application?.status === "pending"
            ? "Awaiting fighter license approval"
            : application?.status === "rejected"
              ? "Fighter license not approved — resubmit required"
              : "Submit your Fighter License application to unlock credentials"}
        </p>
      ) : (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-red-300 lg:text-left">
          Official JTGC Fighter License • {identity.memberId}
        </p>
      )}
    </div>
  );
