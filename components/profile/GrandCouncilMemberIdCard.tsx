...
function CardShell({ children, rotateY = 0 }: { children: ReactNode; rotateY?: number }) {
  return (
    <div
      className="absolute inset-0 overflow-hidden rounded-[0.85rem] border border-[#FF1010]/25 shadow-[0_24px_70px_rgba(0,0,0,0.45)] [backface-visibility:hidden]"
...
function MemberCardFront({
  ...
  const councilPosition = showDetails && application ? answers.councilPosition || application.positionTitle || "—" : "—";
  const province = showDetails && application ? answers.councilProvince || application.addressProvince || "—" : "—";
  const appointmentDate =
    showDetails && application ? formatLicenseDate(answers.appointmentDate) : preview ? "Pending" : "—";
  const yearsInService = showDetails && application ? answers.yearsInService || "—" : "—";
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between bg-black px-3 py-2 text-white">
          <div>
            ...
            <p className="text-[0.55rem] font-semibold tracking-[0.18em] text-red-300">GRAND COUNCIL PH</p>
          </div>
          <div
            aria-hidden
        ...
        <div className="relative flex-1 overflow-hidden bg-[#ececec] p-3">
          ...
          <div className="relative space-y-2">
            ...
            <FieldBlock accent label="Council Position" value={councilPosition} />
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Province" value={province} />
              <FieldBlock label="Years in Service" value={yearsInService} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Appointment Date" value={appointmentDate} />
              <FieldBlock label="Member ID" value={idNumber} />
            </div>
          </div>
        </div>

        <div className="bg-[#FF1010] px-4 py-1.5">
          <p className="font-display text-[1.15rem] italic leading-none tracking-[0.08em] text-white">Grand Council Member</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-[#f2f2f2] text-black">
      <div className="bg-black px-3 py-2">
        <p className="text-[0.55rem] font-black uppercase tracking-[0.12em] text-red-300">In Case Of Emergency</p>
      </div>

      <div className="flex-1 px-3 py-3">
        <div className="space-y-1 text-[0.38rem]">
          <BackField label="Name" value={showDetails && application ? application.emergencyContactName : ""} />
          <BackField
            label="Relationship"
            value={showDetails && application ? application.emergencyContactRelationship : ""}
          />
          <BackField label="Address" multiline value={showDetails && application ? application.emergencyContactAddress : ""} />
          <BackField label="Phone Number" value={showDetails && application ? application.emergencyContactPhone : ""} />
        </div>
        <p className="mt-4 text-[0.33rem] leading-[0.52rem] text-black">
          This credential identifies an appointed member of the Juego Todo Grand Council PH. Report loss or damage to
          the JTGC administration immediately.
        </p>
      </div>
      ...
      <div className="border-t border-black/10 bg-black px-3 py-2 text-center">
        <p className="text-[0.34rem] leading-[0.52rem] text-red-100">
          By presenting this card, the member agrees to abide by the rules, regulations, and code of conduct set forth by
          Juego Todo Grand Council PH.
        </p>
      </div>
    </div>
  );
...
export function GrandCouncilMemberIdCard({
  user,
  identity,
  application = null,
  preview = false,
  ...
  return (
    <div className="w-full max-w-[28rem]">
      ...
      >
        ...
        >
          <CardShell>
            <MemberCardFront application={application} approved={approved} preview={preview} user={user} />
          </CardShell>
          <CardShell rotateY={180}>
            <MemberCardBack application={application} approved={approved} preview={preview} />
          </CardShell>
        </div>
      ...
      {preview ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-red-300 lg:text-left">
          Preview — Grand Council Member ID pending approval
        </p>
      ) : !showApprovedState ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-zinc-500 lg:text-left">
          {application?.status === "pending"
            ...
            : application?.status === "rejected"
              ? "Member application not approved — resubmit required"
              : "Submit your Grand Council Member application to unlock credentials"}
        </p>
      ) : (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-red-300 lg:text-left">
          Official Grand Council Member ID • {identity.memberId}
        </p>
      )}
    </div>
  );
