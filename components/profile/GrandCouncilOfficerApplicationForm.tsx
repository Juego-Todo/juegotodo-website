...
type GrandCouncilOfficerApplicationFormProps = {
  user: UserProfile;
  userPhone: string;
...
export function GrandCouncilOfficerApplicationForm({
  user,
  userPhone,
  initialApplication,
  showConfirmationInitially,
}: GrandCouncilOfficerApplicationFormProps) {
  ...
  const [form, setForm] = useState<LicenseApplicationInput>(() => {
    ...
    return {
      ...emptyLicenseApplicationInput,
      applicationProgram: "grand_council_officer",
      restrictionCode: "JT2",
      backgroundAnswers: {
        officerPosition: grandCouncilOfficerPositions[1]?.label ?? "JT2. G.C.O",
      },
  ...
  const previewIdentity = useMemo(
    () =>
      ({
        memberId: application?.idNumber ?? "GCO-PREVIEW",
  ...
  function validateStep3() {
    if (!form.backgroundAnswers.officerPosition?.trim()) return "Position is required.";
    if (!form.backgroundAnswers.officerRegion?.trim()) return "Region is required.";
    if (!form.backgroundAnswers.officerProvince?.trim()) return "Province is required.";
    if (!form.backgroundAnswers.appointmentDate?.trim()) return "Appointment date is required.";
    if (!form.backgroundAnswers.officerOffice?.trim()) return "Office is required.";
    return null;
  }

  function validateStep4() {
    if (!form.backgroundAnswers.yearsOfService?.trim()) return "Years of service is required.";
    if (!form.backgroundAnswers.assignedChapter?.trim()) return "Assigned chapter is required.";
    if (!form.backgroundAnswers.committee?.trim()) return "Committee is required.";
    return null;
  }
  ...
  function handleEditApplication() {
    ...
    setStepError(null);
    setError(null);
    router.replace("/register-for-license/grand-council-officer");
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Unable to submit officer application.");
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <section className="relative mx-auto max-w-2xl py-8 sm:py-10">
          <MotionSection>
            ...
            <div className="glass-panel mt-6 rounded-[1.5rem] p-8 text-center sm:p-10">
              <div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-amber-500/15 text-amber-200">
                <CheckCircle2 size={32} aria-hidden />
              </div>
              <h1 className="font-display mt-6 text-4xl uppercase text-white">Application Submitted</h1>
              <p className="mt-4 text-sm leading-7 text-zinc-400">
                {buildLicenseFullName(form)} • {form.backgroundAnswers.officerPosition ?? getRestrictionLabel(form.restrictionCode)}
              </p>
              <p className="mt-2 text-xs font-black uppercase tracking-[0.16em] text-amber-200">
                Officer ID will be issued as {application.idNumber} upon approval
              </p>
    ...
    <main className="overflow-hidden px-4 pt-24 sm:px-6 sm:pt-28 lg:px-8 lg:pt-32">
      <section className="relative mx-auto max-w-3xl py-8 sm:py-10">
        <MotionSection>
          ...
          <div className="mt-5">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-amber-300">Grand Council Officer</p>
            <h1 className="font-display mt-2 text-3xl uppercase text-white sm:text-4xl">Officer Application</h1>
          </div>
          ...
          <form className="glass-panel mt-6 rounded-[1.5rem] p-5 sm:p-6" onSubmit={handleSubmit}>
            <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4">
              <div>
                <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-amber-300">
              ...
              <div className="hidden gap-1 sm:flex">
                {Array.from({ length: TOTAL_STEPS }, (_, index) => (
                  <span
                    className={`h-1.5 w-6 rounded-full ${step >= index + 1 ? "bg-amber-400" : "bg-white/10"}`}
                    key={index}
                  />
                ))}
              </div>
            ...
            <div className="py-4">
              {step === 1 ? (
                <FormGroup
                  columns={3}
                  description="Enter your legal name and personal details for your official Grand Council Officer credential."
              ...
              {step === 3 ? (
                <FormGroup description="Provide your appointed Grand Council position and jurisdiction." title="Position">
                  <SelectField
                    ...
                    optionLabels={Object.fromEntries(grandCouncilOfficerPositions.map((option) => [option.label, option.label]))}
                    options={grandCouncilOfficerPositions.map((option) => option.label)}
                    required
                    value={form.backgroundAnswers.officerPosition ?? ""}
                  />
                  <SelectField
                    label="Region"
                    onChange={(value) => updateBackgroundField("officerRegion", value)}
                    options={philippineRegions}
                    required
                    value={form.backgroundAnswers.officerRegion ?? ""}
                  />
                  <Field
                    label="Province"
                    onChange={(value) => updateBackgroundField("officerProvince", value)}
                    required
                    value={form.backgroundAnswers.officerProvince ?? ""}
                  />
                  <Field
                  ...
                  <Field
                    className="sm:col-span-2"
                    label="Office"
                    onChange={(value) => updateBackgroundField("officerOffice", value)}
                    placeholder="e.g. Governance & Legal Office"
                    required
                    value={form.backgroundAnswers.officerOffice ?? ""}
                  />
                </FormGroup>
              ) : null}
              ...
              {step === 4 ? (
                <FormGroup description="Summarize your service record and council assignment." title="Organization">
                  <Field
                    ...
                    onChange={(value) => updateBackgroundField("yearsOfService", value)}
                    placeholder="e.g. 8"
                    required
                    value={form.backgroundAnswers.yearsOfService ?? ""}
                  />
                  <Field
                    label="Assigned Chapter"
                    onChange={(value) => updateBackgroundField("assignedChapter", value)}
                    required
                    value={form.backgroundAnswers.assignedChapter ?? ""}
                  />
                  <Field
                    className="sm:col-span-2"
                    label="Committee"
                    onChange={(value) => updateBackgroundField("committee", value)}
                    placeholder="e.g. Rules & Ethics"
                    required
                    value={form.backgroundAnswers.committee ?? ""}
                  />
                </FormGroup>
              ) : null}
              ...
              {step === 5 ? (
                <div className="space-y-4">
                  <FormGroup description="Upload your appointment letter, valid ID, profile photo, and signature." title="Uploads">
                    <FileUploadField
                      accept="image/*,.pdf"
                      className="sm:col-span-2"
                  ...
                  <section className="rounded-2xl border border-amber-400/15 bg-black/20 p-4 sm:p-5">
                    <div className="mb-4 border-b border-white/8 pb-3">
                      <h3 className="text-xs font-black uppercase tracking-[0.18em] text-amber-200">
                        Grand Council Officer ID Preview
                      </h3>
                      <p className="mt-1.5 text-xs leading-5 text-zinc-500">
                        Review your generated officer credential before submission. Drag to rotate the card.
                      </p>
                    </div>
                    <GrandCouncilOfficerIdPreview form={form} identity={previewIdentity} user={user} />
                  </section>
                </div>
              ) : null}
            </div>
            ...
            <div className="space-y-3 border-t border-white/10 pt-4">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {step < TOTAL_STEPS ? (
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-amber-500 px-8 text-xs font-black uppercase tracking-[0.16em] text-black transition hover:bg-amber-400"
                    onClick={handleNext}
                    type="button"
                  >
                    Next
                  ...
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-amber-500 px-8 text-xs font-black uppercase tracking-[0.16em] text-black transition hover:bg-amber-400 disabled:opacity-60"
                    disabled={submitting}
                    type="submit"
                  >
