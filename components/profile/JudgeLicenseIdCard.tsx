...
import { formatJudgeLevels, formatLicenseDate } from "@/data/license-applications";
...
function CardShell({ children, rotateY = 0 }: { children: ReactNode; rotateY?: number }) {
  return (
    <div
      className="absolute inset-0 overflow-hidden rounded-[0.85rem] border border-indigo-500/25 shadow-[0_24px_70px_rgba(0,0,0,0.45)] [backface-visibility:hidden]"
...
function FieldBlock({
  ...
  return (
    <div>
      <p className={`text-[0.48rem] font-black uppercase tracking-[0.08em] text-black ${accent ? "text-indigo-700" : ""}`}>
        {label}
      </p>
      <p
...
function JudgeCardFront({
  ...
  const levels = showDetails && application ? formatJudgeLevels(answers) || "—" : "—";
  const yearsExperience = showDetails && application ? answers.judgeYearsExperience || "—" : "—";
    ...
    <div className="flex h-full bg-white text-black">
      <div className="relative flex w-[34%] flex-col bg-[#efefef] p-2">
        <div className="flex flex-1 items-center justify-center border border-black/15 bg-white">
          {showDetails ? (
            <div className="flex h-full w-full items-center justify-center bg-[linear-gradient(180deg,#6366f1,#4338ca)] font-display text-4xl text-white">
              {initials}
            </div>
          ) : (
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-2">
          ...
          <div className="grid h-10 w-10 place-items-center rounded-sm bg-indigo-700 text-[0.42rem] font-black leading-none text-white">
            JD
          </div>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between bg-indigo-950 px-3 py-2 text-white">
          <div>
            <p className="font-display text-[1.05rem] leading-none tracking-[0.04em]">JUEGO TODO</p>
            <p className="text-[0.55rem] font-semibold tracking-[0.18em] text-indigo-200">JUDGE LICENSE</p>
          </div>
          <div
            aria-hidden
        ...
        <div className="relative flex-1 overflow-hidden bg-[#ececec] p-3">
          <div className="relative space-y-2">
            ...
            <FieldBlock accent label="Judging Levels" value={levels} />
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Judge ID" value={idNumber} />
        ...
        <div className="bg-indigo-800 px-4 py-1.5">
          <p className="font-display text-[1.15rem] italic leading-none tracking-[0.08em] text-white">Judge</p>
        </div>
      </div>
    </div>
  );
...
function JudgeCardBack({
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-[#f2f2f2] text-black">
      <div className="bg-indigo-950 px-3 py-2">
        <p className="text-[0.55rem] font-black uppercase tracking-[0.12em] text-indigo-100">Judging Record</p>
      </div>

      <div className="flex-1 px-3 py-3">
        <div className="space-y-1 text-[0.38rem]">
          <BackField label="Levels" value={showDetails ? formatJudgeLevels(answers) : ""} />
          <BackField label="Years" value={showDetails ? answers.judgeYearsExperience || "" : ""} />
          <BackField label="Certifications" multiline value={showDetails ? answers.judgeCertifications || "" : ""} />
          <BackField label="Events Worked" multiline value={showDetails ? answers.eventsWorked || "" : ""} />
      ...
      <div className="border-t border-black/10 bg-indigo-950 px-3 py-2 text-center">
        <p className="text-[0.34rem] leading-[0.52rem] text-indigo-100">
          Authorized JTGC judge credential. Valid for sanctioned Juego Todo competition scoring and adjudication.
        </p>
      </div>
    </div>
  );
...
export function JudgeLicenseIdCard({
  user,
  identity,
  application = null,
  preview = false,
  ...
  return (
    <div className="w-full max-w-[28rem]">
      ...
      >
        ...
        >
          <CardShell>
            <JudgeCardFront application={application} approved={approved} preview={preview} user={user} />
          </CardShell>
          <CardShell rotateY={180}>
            <JudgeCardBack application={application} approved={approved} preview={preview} />
          </CardShell>
        </div>
      </div>

      {preview ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-indigo-300 lg:text-left">
          Preview — Judge License pending approval
        </p>
      ) : !showApprovedState ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-zinc-500 lg:text-left">
          {application?.status === "pending"
            ? "Awaiting judge license approval"
            : application?.status === "rejected"
              ? "Judge license not approved — resubmit required"
              : "Submit your Judge License application to unlock credentials"}
        </p>
      ) : (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-indigo-300 lg:text-left">
          Official JTGC Judge License • {identity.memberId}
        </p>
      )}
    </div>
  );
