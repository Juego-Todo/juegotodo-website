...
import { formatLicenseDate, formatRefereeLevels, formatRefereeRulesets } from "@/data/license-applications";
...
function CardShell({ children, rotateY = 0 }: { children: ReactNode; rotateY?: number }) {
  return (
    <div
      className="absolute inset-0 overflow-hidden rounded-[0.85rem] border border-amber-500/25 shadow-[0_24px_70px_rgba(0,0,0,0.45)] [backface-visibility:hidden]"
...
function RefereeCardFront({
  ...
  const levels = showDetails && application ? formatRefereeLevels(answers) || "—" : "—";
  const rulesets = showDetails && application ? formatRefereeRulesets(answers) || "—" : "—";
  const yearsExperience = showDetails && application ? answers.refereeYearsExperience || "—" : "—";
    ...
    <div className="flex h-full bg-white text-black">
      <div className="relative flex w-[34%] flex-col bg-[#efefef] p-2">
        <div className="flex flex-1 items-center justify-center border border-black/15 bg-white">
          {showDetails ? (
            <div className="flex h-full w-full items-center justify-center bg-[linear-gradient(180deg,#f59e0b,#d97706)] font-display text-4xl text-white">
              {initials}
            </div>
          ) : (
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-2">
          ...
          <div className="grid h-10 w-10 place-items-center rounded-sm bg-amber-600 text-[0.42rem] font-black leading-none text-white">
            RF
          </div>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between bg-amber-950 px-3 py-2 text-white">
          <div>
            <p className="font-display text-[1.05rem] leading-none tracking-[0.04em]">JUEGO TODO</p>
            <p className="text-[0.55rem] font-semibold tracking-[0.18em] text-amber-200">REFEREE LICENSE</p>
          </div>
          <div
            aria-hidden
        ...
        <div className="relative flex-1 overflow-hidden bg-[#ececec] p-3">
          <div className="relative space-y-2">
            ...
            <FieldBlock accent label="Officiating Levels" value={levels} />
            <FieldBlock label="Rulesets Certified" value={rulesets} />
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Referee ID" value={idNumber} />
        ...
        <div className="bg-amber-700 px-4 py-1.5">
          <p className="font-display text-[1.15rem] italic leading-none tracking-[0.08em] text-white">Referee</p>
        </div>
      </div>
    </div>
  );
...
function RefereeCardBack({
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-[#f2f2f2] text-black">
      <div className="bg-amber-950 px-3 py-2">
        <p className="text-[0.55rem] font-black uppercase tracking-[0.12em] text-amber-100">Officiating Record</p>
      </div>

      <div className="flex-1 px-3 py-3">
        <div className="space-y-1 text-[0.38rem]">
          <BackField label="Levels" value={showDetails ? formatRefereeLevels(answers) : ""} />
          <BackField label="Rulesets" value={showDetails ? formatRefereeRulesets(answers) : ""} />
          <BackField label="Certifications" multiline value={showDetails ? answers.refereeCertifications || "" : ""} />
          <BackField label="Previous Events" multiline value={showDetails ? answers.previousEvents || "" : ""} />
      ...
      <div className="border-t border-black/10 bg-amber-950 px-3 py-2 text-center">
        <p className="text-[0.34rem] leading-[0.52rem] text-amber-100">
          Authorized JTGC referee credential. Valid for sanctioned Juego Todo competition officiating.
        </p>
      </div>
    </div>
  );
...
export function RefereeLicenseIdCard({
  user,
  identity,
  application = null,
  preview = false,
  ...
  return (
    <div className="w-full max-w-[28rem]">
      ...
      >
        ...
        >
          <CardShell>
            <RefereeCardFront application={application} approved={approved} preview={preview} user={user} />
          </CardShell>
          <CardShell rotateY={180}>
            <RefereeCardBack application={application} approved={approved} preview={preview} />
          </CardShell>
        </div>
      </div>
      ...
      {preview ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-amber-300 lg:text-left">
          Preview — Referee License pending approval
        </p>
      ) : !showApprovedState ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-zinc-500 lg:text-left">
          {application?.status === "pending"
            ? "Awaiting referee license approval"
            : application?.status === "rejected"
              ? "Referee license not approved — resubmit required"
              : "Submit your Referee License application to unlock credentials"}
        </p>
      ) : (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-amber-300 lg:text-left">
          Official JTGC Referee License • {identity.memberId}
        </p>
      )}
    </div>
  );
