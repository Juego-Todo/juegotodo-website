...
const stepTitles = ["Personal Information", "Coaching Information", "Achievements", "Uploads & Preview"] as const;

type FormStep = 1 | 2 | 3 | 4;

type SeniorCoachLicenseApplicationFormProps = {
  user: UserProfile;
  userPhone: string;
...
export function SeniorCoachLicenseApplicationForm({
  user,
  userPhone,
  initialApplication,
  showConfirmationInitially,
}: SeniorCoachLicenseApplicationFormProps) {
  ...
  const [form, setForm] = useState<LicenseApplicationInput>(() => {
    ...
    return {
      ...emptyLicenseApplicationInput,
      applicationProgram: "senior_coach_license",
  ...
  const previewIdentity = useMemo(
    () =>
      ({
        memberId: application?.idNumber ?? "SCO-PREVIEW",
  ...
  function validateStep2() {
    if (!form.backgroundAnswers.highestRank?.trim()) return "Highest rank is required.";
    if (!form.backgroundAnswers.yearsCoaching?.trim()) return "Years coaching is required.";
    if (!form.backgroundAnswers.coachingSchool?.trim()) return "School is required.";
    if (!form.backgroundAnswers.coachingProvince?.trim()) return "Province is required.";
    return null;
  }

  function validateStep3() {
    if (!form.backgroundAnswers.championsProduced?.trim()) return "Champions produced is required.";
    if (!form.backgroundAnswers.seniorCoachCertifications?.trim()) return "Certifications is required.";
    return null;
  }
  ...
  function handleEditApplication() {
    ...
    setStepError(null);
    setError(null);
    router.replace("/register-for-license/senior-coach");
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Unable to submit senior coach license application.");
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <section className="relative mx-auto max-w-2xl py-8 sm:py-10">
          <MotionSection>
            ...
            <div className="glass-panel mt-6 rounded-[1.5rem] p-8 text-center sm:p-10">
              <div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-yellow-500/15 text-yellow-300">
                <CheckCircle2 size={32} aria-hidden />
              </div>
              <h1 className="font-display mt-6 text-4xl uppercase text-white">Application Submitted</h1>
              <p className="mt-4 text-sm leading-7 text-zinc-400">
                {buildLicenseFullName(form)} • {form.backgroundAnswers.highestRank}
              </p>
              <p className="mt-2 text-xs font-black uppercase tracking-[0.16em] text-yellow-300">
                Senior Coach License ID will be issued as {application.idNumber} upon approval
              </p>
              ...
              {!isApproved ? (
                <button
                  className="mt-6 inline-flex min-h-11 items-center justify-center rounded-full border border-white/15 px-8 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:border-yellow-400/40 hover:bg-white/5"
    ...
    <main className="overflow-hidden px-4 pt-24 sm:px-6 sm:pt-28 lg:px-8 lg:pt-32">
      <section className="relative mx-auto max-w-3xl py-8 sm:py-10">
        <MotionSection>
          ...
          <div className="mt-5">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-yellow-400">JTGC Senior Coach License</p>
            <h1 className="font-display mt-2 text-3xl uppercase text-white sm:text-4xl">Senior Coach Application</h1>
          </div>
          ...
          <form className="glass-panel mt-6 rounded-[1.5rem] p-5 sm:p-6" onSubmit={handleSubmit}>
            <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4">
              <div>
                <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-yellow-400">
              ...
              <div className="hidden gap-1 sm:flex">
                {Array.from({ length: TOTAL_STEPS }, (_, index) => (
                  <span
                    className={`h-1.5 w-6 rounded-full ${step >= index + 1 ? "bg-yellow-500" : "bg-white/10"}`}
                    key={index}
                  />
                ))}
              </div>
            ...
            <div className="py-4">
              {step === 1 ? (
                <FormGroup
                  columns={3}
                  description="Enter your personal details for your official JTGC senior coach license."
              ...
              {step === 2 ? (
                <FormGroup description="Provide your coaching rank, experience, school, and province." title="Coaching Information">
                  <SelectField
                    className="sm:col-span-2"
                    label="Highest Rank"
                    onChange={(value) => updateBackgroundField("highestRank", value)}
                    options={seniorCoachRankOptions}
                    required
                    value={form.backgroundAnswers.highestRank ?? ""}
                  />
                  <Field
                  ...
                  <Field
                    className="sm:col-span-2"
                    label="School"
                    onChange={(value) => updateBackgroundField("coachingSchool", value)}
                    placeholder="Affiliated school, gym, or coaching academy"
                    required
                    value={form.backgroundAnswers.coachingSchool ?? ""}
                  />
                  <Field
                    label="Province"
              ...
              {step === 3 ? (
                <FormGroup description="Summarize your championship coaching record and credentials." title="Achievements">
                  <Field
                    className="sm:col-span-2"
                    label="Champions Produced"
                    onChange={(value) => updateBackgroundField("championsProduced", value)}
                    placeholder="e.g. 8 national champions, 3 international medalists"
                    required
                    value={form.backgroundAnswers.championsProduced ?? ""}
                  />
                  <TextAreaField
                    className="sm:col-span-2"
                    label="Certifications"
                    onChange={(value) => updateBackgroundField("seniorCoachCertifications", value)}
                    placeholder="List your senior coaching certifications, master instructor credentials, and completed programs"
                    required
                    value={form.backgroundAnswers.seniorCoachCertifications ?? ""}
                  />
                </FormGroup>
              ) : null}
              ...
              {step === 4 ? (
                <div className="space-y-4">
                  <FormGroup description="Upload your coaching certificates, government ID, and signature." title="Uploads">
                    <FileUploadField
                      accept="image/*,.pdf"
                      className="sm:col-span-2"
                  ...
                  <section className="rounded-2xl border border-yellow-500/15 bg-black/20 p-4 sm:p-5">
                    <div className="mb-4 border-b border-white/8 pb-3">
                      <h3 className="text-xs font-black uppercase tracking-[0.18em] text-yellow-300">Senior Coach License Preview</h3>
                      <p className="mt-1.5 text-xs leading-5 text-zinc-500">
                        Review your official JTGC senior coach credential before submission. Drag to rotate the card.
                      </p>
                    </div>
                    <SeniorCoachLicenseIdPreview form={form} identity={previewIdentity} user={user} />
                  </section>
                </div>
              ) : null}
            </div>
            ...
            <div className="space-y-3 border-t border-white/10 pt-4">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {step < TOTAL_STEPS ? (
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-yellow-600 px-8 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:bg-yellow-500"
                    onClick={handleNext}
                    type="button"
                  >
                    Next
                  ...
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-yellow-600 px-8 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:bg-yellow-500 disabled:opacity-60"
                    disabled={submitting}
                    type="submit"
                  >
