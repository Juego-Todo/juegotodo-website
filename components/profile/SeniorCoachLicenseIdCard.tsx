...
function CardShell({ children, rotateY = 0 }: { children: ReactNode; rotateY?: number }) {
  return (
    <div
      className="absolute inset-0 overflow-hidden rounded-[0.85rem] border border-yellow-500/30 shadow-[0_24px_70px_rgba(0,0,0,0.45)] [backface-visibility:hidden]"
...
function FieldBlock({
  ...
  return (
    <div>
      <p className={`text-[0.48rem] font-black uppercase tracking-[0.08em] text-black ${accent ? "text-yellow-700" : ""}`}>
        {label}
      </p>
      <p
...
function SeniorCoachCardFront({
  ...
  const highestRank = showDetails && application ? answers.highestRank || application.positionTitle || "—" : "—";
  const school = showDetails && application ? answers.coachingSchool || application.fightTeam || "—" : "—";
  const province = showDetails && application ? answers.coachingProvince || "—" : "—";
  const yearsCoaching = showDetails && application ? answers.yearsCoaching || "—" : "—";
    ...
    <div className="flex h-full bg-white text-black">
      <div className="relative flex w-[34%] flex-col bg-[#efefef] p-2">
        <div className="flex flex-1 items-center justify-center border border-black/15 bg-white">
          {showDetails ? (
            <div className="flex h-full w-full items-center justify-center bg-[linear-gradient(180deg,#ca8a04,#854d0e)] font-display text-4xl text-white">
              {initials}
            </div>
          ) : (
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-2">
          ...
          <div className="grid h-10 w-10 place-items-center rounded-sm bg-yellow-600 text-[0.42rem] font-black leading-none text-white">
            SC
          </div>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between bg-slate-950 px-3 py-2 text-white">
          <div>
            ...
            <p className="text-[0.55rem] font-semibold tracking-[0.18em] text-yellow-200">SENIOR COACH LICENSE</p>
          </div>
          <div
            aria-hidden
        ...
        <div className="relative flex-1 overflow-hidden bg-[#ececec] p-3">
          <div className="relative space-y-2">
            ...
            <FieldBlock accent label="Highest Rank" value={highestRank} />
            <FieldBlock label="School" value={school} />
            <FieldBlock label="Province" value={province} />
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Coach ID" value={idNumber} />
              <FieldBlock label="Years Coaching" value={yearsCoaching} />
            </div>
            <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-yellow-700 px-4 py-1.5">
          <p className="font-display text-[1.15rem] italic leading-none tracking-[0.08em] text-white">Senior Coach</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-[#f2f2f2] text-black">
      <div className="bg-slate-950 px-3 py-2">
        <p className="text-[0.55rem] font-black uppercase tracking-[0.12em] text-yellow-100">Coaching Achievements</p>
      </div>

      <div className="flex-1 px-3 py-3">
        <div className="space-y-1 text-[0.38rem]">
          <BackField label="Champions Produced" value={showDetails ? answers.championsProduced || "" : ""} />
          <BackField label="Certifications" multiline value={showDetails ? answers.seniorCoachCertifications || "" : ""} />
      ...
      <div className="border-t border-black/10 bg-slate-950 px-3 py-2 text-center">
        <p className="text-[0.34rem] leading-[0.52rem] text-yellow-100">
          Authorized JTGC senior coach credential. Valid for elite athlete development and championship preparation.
        </p>
      </div>
    </div>
  );
...
export function SeniorCoachLicenseIdCard({
  user,
  identity,
  application = null,
  preview = false,
  ...
  return (
    <div className="w-full max-w-[28rem]">
      ...
      >
        ...
        >
          <CardShell>
            <SeniorCoachCardFront application={application} approved={approved} preview={preview} user={user} />
          </CardShell>
          <CardShell rotateY={180}>
            <SeniorCoachCardBack application={application} approved={approved} preview={preview} />
          </CardShell>
        </div>
      </div>

      {preview ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-yellow-300 lg:text-left">
          Preview — Senior Coach License pending approval
        </p>
      ) : !showApprovedState ? (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-zinc-500 lg:text-left">
          {application?.status === "pending"
            ? "Awaiting senior coach license approval"
            : application?.status === "rejected"
              ? "Senior coach license not approved — resubmit required"
              : "Submit your Senior Coach License application to unlock credentials"}
        </p>
      ) : (
        <p className="mt-4 text-center text-[0.62rem] font-black uppercase tracking-[0.16em] text-yellow-300 lg:text-left">
          Official JTGC Senior Coach License • {identity.memberId}
        </p>
      )}
    </div>
  );
