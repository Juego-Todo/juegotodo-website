...
function CardShell({ children, rotateY = 0 }: { children: ReactNode; rotateY?: number }) {
  return (
    <div
      className="absolute inset-0 overflow-hidden rounded-[0.85rem] border border-zinc-500/25 shadow-[0_24px_70px_rgba(0,0,0,0.45)] [backface-visibility:hidden]"
...
function FieldBlock({
  ...
  return (
    <div>
      <p className={`text-[0.48rem] font-black uppercase tracking-[0.08em] text-black ${accent ? "text-zinc-700" : ""}`}>
        {label}
      </p>
      <p
...
function StaffCardFront({
  ...
  const department = showDetails && application ? answers.staffDepartment || "—" : "—";
  const position = showDetails && application ? answers.staffPosition || application.positionTitle || "—" : "—";
  const office = showDetails && application ? answers.staffOffice || application.fightTeam || "—" : "—";
  const yearsOfService = showDetails && application ? answers.staffYearsOfService || "—" : "—";
    ...
    <div className="flex h-full bg-white text-black">
      <div className="relative flex w-[34%] flex-col bg-[#efefef] p-2">
        <div className="flex flex-1 items-center justify-center border border-black/15 bg-white">
          {showDetails ? (
            <div className="flex h-full w-full items-center justify-center bg-[linear-gradient(180deg,#52525b,#27272a)] font-display text-4xl text-white">
              {initials}
            </div>
          ) : (
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-2">
          ...
          <div className="grid h-10 w-10 place-items-center rounded-sm bg-zinc-700 text-[0.42rem] font-black leading-none text-white">
            ST
          </div>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between bg-zinc-900 px-3 py-2 text-white">
          <div>
            ...
            <p className="text-[0.55rem] font-semibold tracking-[0.18em] text-zinc-300">STAFF LICENSE</p>
          </div>
          <div
            aria-hidden
        ...
        <div className="relative flex-1 overflow-hidden bg-[#ececec] p-3">
          <div className="relative space-y-2">
            ...
            <FieldBlock label="Department" value={department} />
            <FieldBlock label="Office" value={office} />
            <div className="grid grid-cols-2 gap-2">
              <FieldBlock label="Staff ID" value={idNumber} />
              <FieldBlock label="Years of Service" value={yearsOfService} />
            </div>
            <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-zinc-800 px-4 py-1.5">
          <p className="font-display text-[1.15rem] italic leading-none tracking-[0.08em] text-white">Staff</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-[#f2f2f2] text-black">
      <div className="bg-zinc-900 px-3 py-2">
        <p className="text-[0.55rem] font-black uppercase tracking-[0.12em] text-zinc-200">Staff Credentials</p>
      </div>
      ...
      <div className="flex-1 px-3 py-3">
        <div className="space-y-1 text-[0.38rem]">
          ...
          <BackField label="Emergency Contact" value={showDetails && application ? application.emergencyContactName || "" : ""} />
          <BackField label="Relationship" value={showDetails && application ? application.emergencyContactRelationship || "" : ""} />
          <BackField label="Emergency Phone" value={showDetails && application ? application.emergencyContactPhone || "" : ""} />
          <BackField label="Emergency Address" value={showDetails && application ? application.emergencyContactAddress || "" : ""} />
        </div>
      </div>
      ...
      <div className="border-t border-black/10 bg-zinc-900 px-3 py-2 text-center">
        <p className="text-[0.34rem] leading-[0.52rem] text-zinc-300">
          Authorized JTGC staff credential. Valid only for official Juego Todo league operations and event access.
        </p>
      </div>
    </div>
  );
...
export function StaffLicenseIdCard({
  user,
  identity,
  application = null,
  preview = false,
  ...
  return (
    <div className="w-full max-w-[28rem]">
      ...
      >
        ...
        >
          <CardShell>
            <StaffCardFront application={application} approved={approved} preview={preview} user={user} />
          </CardShell>
          <CardShell rotateY={180}>
            <StaffCardBack application={application} approved={approved} preview={preview} />
          </CardShell>
        </div>
      </div>
