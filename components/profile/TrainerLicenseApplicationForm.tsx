...
const stepTitles = ["Personal Information", "Training Information", "Experience", "Uploads & Preview"] as const;

type FormStep = 1 | 2 | 3 | 4;

type TrainerLicenseApplicationFormProps = {
  user: UserProfile;
  userPhone: string;
...
export function TrainerLicenseApplicationForm({
  user,
  userPhone,
  initialApplication,
  showConfirmationInitially,
}: TrainerLicenseApplicationFormProps) {
  ...
  const [form, setForm] = useState<LicenseApplicationInput>(() => {
    ...
    return {
      ...emptyLicenseApplicationInput,
      applicationProgram: "trainer_license",
      restrictionCode: "JT7",
  ...
  const previewIdentity = useMemo(
    () =>
      ({
        memberId: application?.idNumber ?? "TRN-PREVIEW",
  ...
  function toggleTrainingDiscipline(fieldId: string, checked: boolean) {
  ...
  function validateStep2() {
    if (!hasTrainerDisciplineSelected(form.backgroundAnswers)) {
      return "Select at least one training discipline.";
    }
    return null;
  }
  ...
  function validateStep3() {
    ...
    if (!form.backgroundAnswers.trainingGym?.trim()) return "Gym is required.";
    if (!form.backgroundAnswers.certifications?.trim()) return "Certifications is required.";
    return null;
  }

  function validateStep4() {
    if (!form.uploads.trainingCertifications) return "Certifications upload is required.";
  ...
  function handleEditApplication() {
    ...
    setStepError(null);
    setError(null);
    router.replace("/register-for-license/trainer");
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Unable to submit trainer license application.");
    } finally {
      setSubmitting(false);
    }
  }
          ...
          <MotionSection>
            ...
            <div className="glass-panel mt-6 rounded-[1.5rem] p-8 text-center sm:p-10">
              ...
              <p className="mt-4 text-sm leading-7 text-zinc-400">
                {buildLicenseFullName(form)} • JT7 Trainer
              </p>
              <p className="mt-2 text-xs font-black uppercase tracking-[0.16em] text-emerald-300">
                Trainer License ID will be issued as {application.idNumber} upon approval
              </p>
              ...
              {!isApproved ? (
                <button
                  className="mt-6 inline-flex min-h-11 items-center justify-center rounded-full border border-white/15 px-8 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:border-emerald-400/40 hover:bg-white/5"
    ...
    <main className="overflow-hidden px-4 pt-24 sm:px-6 sm:pt-28 lg:px-8 lg:pt-32">
      <section className="relative mx-auto max-w-3xl py-8 sm:py-10">
        <MotionSection>
          ...
          <div className="mt-5">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-emerald-400">JTGC Trainer License</p>
            <h1 className="font-display mt-2 text-3xl uppercase text-white sm:text-4xl">Trainer Application</h1>
          </div>
          ...
          <form className="glass-panel mt-6 rounded-[1.5rem] p-5 sm:p-6" onSubmit={handleSubmit}>
            <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4">
              <div>
                <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-emerald-400">
              ...
              <div className="hidden gap-1 sm:flex">
                {Array.from({ length: TOTAL_STEPS }, (_, index) => (
                  <span
                    className={`h-1.5 w-6 rounded-full ${step >= index + 1 ? "bg-emerald-500" : "bg-white/10"}`}
                    key={index}
                  />
                ))}
              </div>
            ...
            <div className="py-4">
              {step === 1 ? (
                <FormGroup
                  columns={3}
                  description="Enter your personal details for your official JTGC trainer license."
              ...
              {step === 2 ? (
                <FormGroup
                  columns={2}
                  description="Select the training disciplines you are qualified to deliver."
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <div className="mt-3 grid gap-3 sm:grid-cols-2">
                      {trainerDisciplineOptions.map((discipline) => (
                        <label
                          className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-black/30 px-4 py-3 transition hover:border-emerald-400/30"
                          key={discipline.id}
                        >
                          <input
                            checked={form.backgroundAnswers[discipline.id] === "yes"}
                            className="h-4 w-4 rounded border-white/20 bg-black/40 text-emerald-500 focus:ring-emerald-500/40"
                            onChange={(event) => toggleTrainingDiscipline(discipline.id, event.target.checked)}
                            type="checkbox"
                          />
              ...
              {step === 3 ? (
                <FormGroup description="Summarize your training experience and qualifications." title="Experience">
                  <Field
                  ...
                  <Field
                    label="Gym"
                    onChange={(value) => updateBackgroundField("trainingGym", value)}
                    placeholder="Affiliated gym or training facility"
                    required
                    value={form.backgroundAnswers.trainingGym ?? ""}
                  />
                  <TextAreaField
                    className="sm:col-span-2"
                    label="Certifications"
                    onChange={(value) => updateBackgroundField("certifications", value)}
                    placeholder="List your trainer certifications, credentials, and completed programs"
                    required
              ...
              {step === 4 ? (
                <div className="space-y-4">
                  <FormGroup description="Upload your certification documents, government ID, and signature." title="Uploads">
                    <FileUploadField
                      accept="image/*,.pdf"
                      className="sm:col-span-2"
                      label="Certifications"
                      onFileSelect={(file) => updateUploadField("trainingCertifications", file)}
                      required
                      value={form.uploads.trainingCertifications}
                    />
                    <FileUploadField
                      accept="image/*,.pdf"
                      label="Government ID"
                  ...
                  <section className="rounded-2xl border border-emerald-500/15 bg-black/20 p-4 sm:p-5">
                    <div className="mb-4 border-b border-white/8 pb-3">
                      <h3 className="text-xs font-black uppercase tracking-[0.18em] text-emerald-300">Trainer License Preview</h3>
                      <p className="mt-1.5 text-xs leading-5 text-zinc-500">
                        Review your official JTGC trainer credential before submission. Drag to rotate the card.
                      </p>
                    </div>
                    <TrainerLicenseIdPreview form={form} identity={previewIdentity} user={user} />
                  </section>
                </div>
              ) : null}
            </div>
            ...
            <div className="space-y-3 border-t border-white/10 pt-4">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {step < TOTAL_STEPS ? (
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-emerald-600 px-8 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:bg-emerald-500"
                    onClick={handleNext}
                    type="button"
                  >
                    Next
                  ...
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full bg-emerald-600 px-8 text-xs font-black uppercase tracking-[0.16em] text-white transition hover:bg-emerald-500 disabled:opacity-60"
                    disabled={submitting}
                    type="submit"
                  >
